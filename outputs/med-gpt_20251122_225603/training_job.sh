#!/bin/bash
#SBATCH --job-name=med-gpt
#SBATCH --output=med-gpt%j.out
#SBATCH --error=med-gpt%j.err
...
OUTPUT_DIR="outputs/med-gpt_$(date +'%Y%m%d_%H%M%S')"
mkdir -p "$OUTPUT_DIR"
...
mv med-gpt_${SLURM_JOB_ID}.out "$OUTPUT_DIR/" 2>/dev/null || true
mv med-gpt_${SLURM_JOB_ID}.err "$OUTPUT_DIR/" 2>/dev/null || true
