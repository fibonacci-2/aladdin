#!/bin/bash
#SBATCH --job-name=tr-2.6.2
#SBATCH --output=train_2.6.2_%j.out
#SBATCH --error=train_2.6.2_%j.err
...
OUTPUT_DIR="outputs/2.6.2_$(date +'%Y%m%d_%H%M%S')"
mkdir -p "$OUTPUT_DIR"
...
torchrun --standalone  alladin-medium.py \
