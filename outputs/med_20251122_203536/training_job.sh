#!/bin/bash
#SBATCH --job-name=med
#SBATCH --output=med%j.out
#SBATCH --error=med%j.err
...
OUTPUT_DIR="outputs/med_$(date +'%Y%m%d_%H%M%S')"
mkdir -p "$OUTPUT_DIR"
...
torchrun --standalone --nproc_per_node="$GPUS_PER_NODE" alladin-medium.py \
...
mv med_${SLURM_JOB_ID}.out "$OUTPUT_DIR/" 2>/dev/null || true
mv med_${SLURM_JOB_ID}.err "$OUTPUT_DIR/" 2>/dev/null || true
